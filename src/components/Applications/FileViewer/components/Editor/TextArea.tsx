...
export const TextArea = forwardRef<TextAreaRef, TextAreaProps>(
  (
    { content, cursorPosition, settings, onContentChange, onCursorPositionChange, onSelectionChange, className = '' },
    ref
  ) => {
        ...
        if (key === 'Tab') {
          ...
          if (shiftKey) {
            ...
            if (currentLine.startsWith(tabString)) {
              const newContent =
                beforeCursor.slice(0, -currentLine.length) + currentLine.substring(tabString.length) + afterCursor;
              onContentChange(newContent);
            }
          } else {
            // Tab: Indent
            const newContent = value.substring(0, selectionStart) + tabString + value.substring(selectionEnd);
            onContentChange(newContent);
    ...
    const insertText = useCallback(
      (text: string) => {
        ...
        const { selectionStart, selectionEnd, value } = textareaRef.current;
        const newContent = value.substring(0, selectionStart) + text + value.substring(selectionEnd);
