...
export function useVimState(filename?: string, context?: VimEditorContext) {
  ...
  const updateBuffer = useCallback(
    (newBuffer: string[]) => {
      setState((prev) => {
        ...
        const hasChanges =
          prev.buffer.length !== newBuffer.length || prev.buffer.some((line, index) => line !== newBuffer[index]);
  ...
  const executeCommand = useCallback(
    async (commandText: string): Promise<OperationResult> => {
      if (!commandText.trim()) {
      ...
      // Find command
      const command = vimCommands.find((cmd) => cmd.name === commandName || cmd.aliases.includes(commandName));
  ...
  const undo = useCallback(() => {
    setState((prev) => {
      ...
      // Apply the change
      const newState = { ...prev, buffer: newBuffer };
